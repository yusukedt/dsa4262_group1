...
if __name__ == "__main__":
    ...
    agg_data = (
        ...
        # Unlike the features, do not aggregate gene_id and sequence columns (store the first occurrence of each column)
          "gene_id": "first",
          "sequence": "first",
          **{col: "mean" for col in [f"feature_{i}" for i in range(1, 10)]}  # average of features
          })
    )
    ...
    try:
        y_prob = loaded_model.predict_proba(agg_data.iloc[:, 4:13])  # Example for scikit-learn model
